...
__export(index_exports, {
  buildProjectProfile: () => buildProjectProfile,
  findProjectRoot: () => findProjectRoot,
  loadHarnessProjectConfig: () => loadHarnessProjectConfig,
...
function findProjectRoot(startDir) {
  ...
  while (true) {
    if (ROOT_MARKERS.some((marker) => (0, import_node_fs.existsSync)((0, import_node_path.join)(current, marker)))) return current;
    const parent = (0, import_node_path.dirname)(current);
    if (parent === current) return (0, import_node_path.resolve)(startDir);
    current = parent;
  }
}
function buildProjectProfile(projectRoot) {
  const root = (0, import_node_path.resolve)(projectRoot);
...
function loadProjectInstructions(projectRoot) {
  ...
  const add = (path, priority) => {
    const fullPath = (0, import_node_path.join)(root, path);
    if (!(0, import_node_fs.existsSync)(fullPath)) return;
    try {
      files.push({ path, priority, content: (0, import_node_fs.readFileSync)(fullPath, "utf-8") });
    } catch {
    }
  };
  add("KOVA.md", 100);
  ...
  add("CLAUDE.md", 80);
  add("RULES.md", 70);
  const rulesDir = (0, import_node_path.join)(root, ".kova", "rules");
  if ((0, import_node_fs.existsSync)(rulesDir)) {
...
function loadHarnessProjectConfig(projectRoot) {
  const filePath = (0, import_node_path.join)(projectRoot, ".kova", "harness.json");
...
function walk(root, relDir, out, depth) {
  if (depth > 6) return;
  const dir = (0, import_node_path.join)(root, relDir);
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    const full = (0, import_node_path.join)(root, rel);
    try {
...
function collectSignals(root, files) {
  ...
  for (const file of files) {
    const base = (0, import_node_path.basename)(file);
...
function detectWorkspaces(root, files) {
  ...
  const manifests = files.filter((file) => {
    const base = (0, import_node_path.basename)(file);
    return (0, import_node_path.dirname)(file) !== "." && (base in MANIFESTS || base.endsWith(".csproj") || base.endsWith(".sln"));
  });
  const byPath = /* @__PURE__ */ new Map();
  for (const manifest of manifests) {
    const dir = (0, import_node_path.dirname)(manifest).replace(/\\/g, "/");
  ...
  return [...byPath.entries()].map(([path]) => {
    ...
    const scopedSignals = collectSignals((0, import_node_path.join)(root, path), scopedFiles);
    ...
    return {
      ...
      frameworks: detectFrameworks((0, import_node_path.join)(root, path), scopedFiles),
...
function readPackageJson(root, scope = "") {
  const path = (0, import_node_path.join)(root, scope, "package.json");
...
function isSensitiveProjectFile(file) {
  const base = (0, import_node_path.basename)(file).toLowerCase();
...
function listMarkdownFiles(dir) {
  try {
    return (0, import_node_fs.readdirSync)(dir).filter((file) => file.endsWith(".md")).sort();
  } catch {
    return [];
  }
}
function isSourceCandidate(root, file) {
  try {
    return (0, import_node_fs.statSync)((0, import_node_path.join)(root, file)).size <= 2e5;
  } catch {
    return false;
  }
}
